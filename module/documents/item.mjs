...
export class NewEraItem extends Item {
  ...
  _prepareBasicItemData(system) {
    ...
    if (system.units) {
      if (system.rollQuantity) {
        system.listDisplayName = `${
          this.name
        } (${NewEraUtils.formatRollExpression(
          system.variableQuantity.dieCount,
          system.variableQuantity.dieSize,
          system.variableQuantity.modifier
        )} ${system.units})`;
      } else {
      ...
      if (system.rollQuantity) {
        system.listDisplayName = `${
          this.name
        } x${NewEraUtils.formatRollExpression(
          system.variableQuantity.dieCount,
          system.variableQuantity.dieSize,
          system.variableQuantity.modifier
        )}`;
  ...
  _prepareEquipmentName(system, type) {
    ...
    this.name = system.useCustomItemName
      ? system.customName
      : this._generateDefaultName(system, type);
    system.listDisplayName = this.name;
  }
  ...
  _generateDefaultName(system, type) {
    ...
    return `${conditionLabel}${system.arcane ? "Arcane " : ""}${qualityLabel}${
      system.material
    } ${baseName}${
      system.enchanted ? ` of ${system.enchantmentDescriptor}` : ""
    }`;
  }
  ...
  _prepareMagicData(system) {
    ...
    system.formattedDescription = NewEraUtils.amplifyAndFormatDescription(
      system.description,
      system.ampFactor,
      "S"
    );
  ...
  _prepareSpellData(system) {
    ...
    if (system.ampFactor > 1) {
      system.amplifiedData = {
        ...
        damage: NewEraUtils.amplifyValue(
  ...
  _preparePotionData(system) {
    ...
    system.formattedDescription = NewEraUtils.amplifyAndFormatDescription(
      system.description,
      system.doses,
      system.stackingBehavior
    );
    system.amplified = system.doses > 1;
    if (system.isRecipe) {
      system.listDisplayName = this.name + " Recipe";
      this.img = `systems/newera-sol366/resources/pr_${
        NEWERA.chantLevels[this.system.recipeLevel]
      }.png`; //Chant levels are the same as potion levels so this works
    } else if (system.quantity > 1) {
      system.listDisplayName = `${this.name} x${system.quantity}`;
    } else {
      system.listDisplayName = this.name;
  ...
  _prepareFeatData(system) {
    ...
    if (system.currentTier > system.maximumTier && !system.isUpgrade)
      system.currentTier = system.maximumTier;
    ...
    if (system.featType == "GF") {
      ...
      this.img = `${NEWERA.images}/chant-${
        NEWERA.chantLevels[system.chantLevel]
      }.png`;
    } else {
      this.img = `systems/newera-sol366/resources/${system.featSubType
        .toLowerCase()
        .replaceAll(" ", "-")}.png`;
    }
    ...
    if (system.isUpgrade) {
      ...
      system.totalCost = total.toString();
      system.displayName =
        this.name + " " + NEWERA.romanNumerals[system.currentTier];
    }
    system.displayCost =
      system.featType == "FL"
        ? "+" + Math.abs(system.totalCost)
        : system.totalCost;
  }
  ...
  _prepareMeleeWeaponData(system) {
    ...
    if (system.weaponType) {
      ...
      system.weight = Math.max(
        weaponStats.weight + materialStats.adjustWeight,
        0
      );
      system.durability =
        materialStats.durability + weaponStats.adjustDurability;
      system.value = NewEraUtils.getEstimatedValue(
        weaponStats.value,
        materialStats.valueMultiplier,
        system.condition,
        system.quality
      );
      system.handedness = weaponStats.handedness;

      let oneHandedHitSkill = system.weaponType.includes("Knuckleduster")
        ? "athletics"
        : "one-handed";
      ...
      if (system.handedness == "1H" || system.handedness == "1.5H") {
        ...
        for (const attack of weaponStats.attackActions) {
          system.attacks.push({
            ...
            damageRoll: `1${
              weaponStats.damage.oneHanded
            }+@abilities.strength.mod+${
              system.quality + materialStats.modifiers.damage
            }`,
      ...
      if (system.handedness == "2H" || system.handedness == "1.5H") {
        ...
        for (const attack of weaponStats.attackActions) {
          system.attacks.push({
            ...
            damageRoll: `${attack.power ? 2 : 1}${
              weaponStats.damage.twoHanded
            }+@abilities.strength.mod+${
              system.quality + materialStats.modifiers.damage
            }`,
  ...
  _prepareArmorData(system) {
    if (system.armorType) {
      ...
      system.weight = Math.max(
        armorStats.weight + materialStats.adjustWeight,
        0
      );
      system.durability =
        materialStats.durability + armorStats.adjustDurability;
      system.value = NewEraUtils.getEstimatedValue(
        armorStats.value,
        materialStats.valueMultiplier,
        system.condition,
      ...
      system.armorRating =
        armorStats.armorRating + materialStats.modifiers.armor;
    } else {
      system.standard = false;
    }
  }
  ...
  _prepareShieldData(system) {
    if (system.shieldType) {
      ...
      system.weight = Math.max(
        shieldStats.weight + materialStats.adjustWeight,
        0
      );
      system.durability =
        materialStats.durability + shieldStats.adjustDurability;
      system.value = NewEraUtils.getEstimatedValue(
        shieldStats.value,
        materialStats.valueMultiplier,
        system.condition,
      ...
      system.shieldRating =
        shieldStats.shieldRating + materialStats.modifiers.shield;
    } else {
      system.standard = false;
    }
  }
  ...
  _prepareCustomActionData(actor, system) {
    for (const roll of Object.values(system.rolls)) {
      ...
      roll.formula =
        roll.customFormula ||
        `${roll.dieCount}d${roll.dieSize}+${roll.modifier}`;
  ...
  _preparePhoneData(actor, system) {
    ...
    if (system.openApp == "call") {
      ...
      for (const actor of game.actors.values()) {
        if (
          actor.type.includes("Character") &&
          actor.name.toLowerCase() ==
            this.system.contacts[system.currentContact].name.toLowerCase()
        ) {
  ...
  _deserializeActionData(system) {
    system.actions = [];
    Object.values(system.actionData).forEach((actionString) =>
      system.actions.push(JSON.parse(actionString))
    );
  }
  ...
  async addMessage(sent, index, content) {
    if (this.type !== "Phone") return;
    const system = this.system;
    const newMessageIndex = system.contacts[index].messages
      ? Object.keys(system.contacts[index].messages).length
      : 0;
    const update = {
      system: {
        contacts: {
  ...
  get totalEnergyCost() {
    if (
      this.typeIs(NewEraItem.Types.ENCHANTMENT) &&
      this.system.enchantmentType == "CE"
    ) {
  ...
  async addActionRoll(index) {
    const system = this.system;
    const newIndex = system.actions[index].rolls
      ? Object.keys(system.actions[index].rolls).length
      : 0;
    const update = {
      system: {
        actions: {
  ...
  async enchant(
    ...
    if (!this.isEnchantmentValid(enchantment.system.validTargets)) {
      ui.notifications.error(
        `${enchantment.name} can't be applied to this item.`
      );
      return;
    }
    ...
    if (caster) {
      const success = await caster.cast(
        enchantment,
        ampFactor,
        noSkillCheck,
        energyPool,
        this.system.enchantabilityModifier
      );
      if (!success) {
        return false;
      }
    ...
    for (const action of Object.values(enchantment.system.actions)) {
      ...
      newActions[
        Object.keys(newActions).length + Object.keys(this.system.actions).length
      ] = action;
    }
    ...
    if (!this.system.enchanted) {
      this.update({
        system: {
          ...
          enchantmentColor:
            NEWERA.enchantmentAuraColors[enchantment.system.school],
    ...
    await this.createEmbeddedDocuments("ActiveEffect", [
      {
        ...
        description: NewEraUtils.amplifyAndFormatDescription(
          enchantment.system.description,
          ampFactor
        ),
        owner: this.uuid,
        disabled: false,
    ...
    ui.notifications.info(
      `Enchanted ${this.name} with ${NewEraUtils.formatSpellName(
        enchantment,
        ampFactor
      )}.`
    );
    ...
    switch (this.type) {
      ...
      case "Item":
        switch (this.system.equipSlot) {
          ...
          case "C":
            return (
              targetData.object || (targetData.ranged && this.isAmmunition())
            );
          case "O":
          case "B":
            return targetData.clothing;
  ...
  isAmmunition() {
    if (this.type != "Item" || this.system.equipSlot != "C") {
      ...
      return !!Object.values(NEWERA.compatibleAmmoIds).find((list) =>
        list.includes(this.system.casperObjectId)
      );
    }
  }
      ...
      if (this.actor && NewEraUtils.sendEquipmentChangeMessages()) {
        if (prevState) {
          this.actor.actionMessage(
            this.img,
            `${NEWERA.images}/ac_adventuring.png`,
            "{NAME} retrieves {d} {0} from storage.",
            this.name
          );
        } else {
          this.actor.actionMessage(
          ...
          for (const ORcondition of ANDcondition) {
            if (ORcondition == {} || ORcondition.check == "none") {
              ...
              if (ORcondition.inverse) {
                if (
                  parseInt(ORcondition.required) >=
                  parseInt(ORcondition.value(actor))
                ) {
  ...
  _tokenizePrerequisites(tier) {
    ...
    if (this.type == "Feat") {
      try {
        const customCondition = ExtendedFeatData.getCustomPrerequisiteData(
          this.system.casperObjectId,
          tier
        );
        if (customCondition) {
      ...
      const prerequisiteString =
        tier == 1
          ? this.system.base.prerequisites
          : this.system.tiers[tier].prerequisites;
      if (prerequisiteString) {
        let prerequisites = [];
        const conditions = prerequisiteString.split(",");
        for (const condition of conditions) {
          //First, break out the expression by commas/semicolons. This step is an AND (each part must evaluate to true)
  ...
  _getPrerequisiteCondition(text) {
    ...
    if (number) {
      //Evaluate the condition as a minimum numeric stat value
      const expr =
        NEWERA.prerequisiteActorStatTextMatching[
          Object.keys(NEWERA.prerequisiteActorStatTextMatching).find((word) =>
      ...
      return {
        ...
        required: number,
        inverse:
          words.filter((word) => inverseKeywords.includes(word)).length > 0,
      };
    } else {
      //Evaluate the condition as requiring a specific feat or feature
      return {
        check: "ability",
        value: text,
      };
  ...
  async durabilityCheck(modifier = 0) {
    ...
    if (roll.total > this.system.durability) {
      ...
      if (this.parent) {
        this.parent.actionMessage(
          this.parent.img,
          this.img,
          "{NAME}'s {0} is {1}",
          this.name,
          NEWERA.conditionChangedDescriptions[newCondition]
        );
      }
      await this.update({
        system: {
  ...
  async printDetails(speaker, ampFactor = 1) {
    ...
    if (this.typeIs(NewEraItem.Types.SPELL)) {
      const description = NewEraUtils.amplifyAndFormatDescription(
        this.system.description,
        ampFactor
      );
      const title = NewEraUtils.formatSpellName(this, ampFactor);
      const range = `${
        this.system.range.value * (this.system.range.scales ? ampFactor : 1)
      } ft ${this.system.range.description}`;
      const castingTime =
        NEWERA.spellCastingTimes[this.system.castType] || this.system.castTime;
      template = `
        <div class="chat-item-details">
          <img src="${this.img}" />
          <h2>${title}</h2>
          <h3>Level <strong>${this.system.level * ampFactor}</strong> ${
  ...
  async deleteMaterialCost(index) {
    await this.update({
      system: {
        materialCosts: NewEraUtils.spliceIndexedObject(
          this.system.materialCosts,
          index
        ),
      },
    });
  ...
  async deleteComponent(index) {
    await this.update({
      system: {
        components: NewEraUtils.spliceIndexedObject(
  ...
  async rollQuantity() {
    if (this.system.rollQuantity) {
      const roll = new Roll(
        `${this.system.variableQuantity.dieCount}d${this.system.variableQuantity.dieSize}+${this.system.variableQuantity.modifier}`,
        this.getRollData()
      );
      await roll.evaluate();
      roll.toMessage({
  ...
  async fire() {
    if (this.typeIs(NewEraItem.Types.RANGED_WEAPON)) {
      if (this.system.firingAction == "SA") {
        ...
        const toShoot = Math.min(
          this.system.firingRate,
          this.system.ammo.loaded
        );
  ...
  async reload(ammo) {
    if (this.typeIs(NewEraItem.Types.RANGED_WEAPON)) {
      if (!ammo || ammo.system.quantity == 0) {
        ...
        const perAction = ["SA", "FA"].includes(this.system.firingAction)
          ? this.system.ammo.clipSize
          : 1;
        ...
        if (ammo.system.quantity == loaded) {
          if (this.actor) {
            await this.actor.deleteItem(ammo.id);
          } else {
            await ammo.delete();
          }
        } else {
  ...
  async cock() {
    if (
      this.typeIs(NewEraItem.Types.RANGED_WEAPON) &&
      this.system.firingAction == "M"
    ) {
      await this.update({
        system: {
          ammo: {
