...
export class NewEraActor extends Actor {
  ...
  _validateResources(system) {
    ...
    if (system.lifePoints) {
      ...
      if (system.lifePoints.value > system.lifePoints.max)
  ...
  _prepareCharacterData(system) {
    ...
    system.characterPoints.cpa = this._calculateCpa(
      system.characterPoints.cpt,
      system.age,
      this.items
    );
    ...
    const maxInspiration =
      difficulty == 0 ? 5 : difficulty == 1 ? 3 : difficulty == 2 ? 1 : 0;
  ...
  _calculateLevel() {
    ...
    const levelThresholds = game.settings.get("newera-sol366", "advancedSkills")
      ? NEWERA.levelThresholds.advanced
      : NEWERA.levelThresholds.standard;

    for (let i = 0; i < 100; i++) {
      let currentLevelThreshold =
        i < 31 ? levelThresholds[i] : 5000 + (i - 30) * 500;
      let nextLevelThreshold =
        i < 30 ? levelThresholds[i + 1] : 5000 + (i - 29) * 500;
      system.levelGap = nextLevelThreshold - currentLevelThreshold;
      if (
        system.characterPoints.cpt >= currentLevelThreshold &&
        system.characterPoints.cpt < nextLevelThreshold
      ) {
        system.level = i;
        system.characterPoints.progressMax =
          nextLevelThreshold - currentLevelThreshold;
        system.characterPoints.progressValue =
          system.characterPoints.cpt - currentLevelThreshold;
  ...
  _calculateInventoryLoad(system, items) {
    ...
    for (const item of items) {
      if (
        item.typeIs(NewEraItem.Types.INVENTORY) &&
        !item.system.stored &&
        typeof item.system.weight != "undefined"
      ) {
      ...
      if (item.typeIs(NewEraItem.Types.ENCHANTABLE) && item.system.enchanted) {
        ...
        if (
          slot &&
          !["backpack", "leftHand", "rightHand"].includes(slot) &&
          !item.typeIs(NewEraItem.Types.WEAPON)
        ) {
          //Count enchanted items, that are not weapons, and are worn on the body
          ench += item.system.totalEnchantmentLevel;
        }
      }
      if (item.typeIs(NewEraItem.Types.ARMOR)) {
        if (
          item.system.armorType == "Chest" ||
          item.system.armorType == "Full Body"
        ) {
          armor += item.system.armorRating;
        }
      }
      if (
        item.system.casperObjectId &&
        NEWERA.moneyItemValues[item.system.casperObjectId]
      ) {
        money +=
          NEWERA.moneyItemValues[item.system.casperObjectId] *
      ...
      if (
        item.system.casperObjectId &&
        NEWERA.ancientMoneyItemValues[item.system.casperObjectId]
      ) {
        ancientMoney +=
          NEWERA.ancientMoneyItemValues[item.system.casperObjectId] *
          item.system.quantity;
  ...
  _prepareCreatureData(system) {
    ...
    this._prepareAbilityScoreModifiers(system);
    system.rollableHP =
      system.hitPoints.max == 0 && system.initialHitPoints.dieCount > 0;
    system.hitPoints.total = system.hitPoints.value;

    system.saves.endurance.mod =
      system.abilities.strength.mod +
      system.abilities.constitution.mod +
      system.saves.endurance.bonus;
    system.saves.reflex.mod =
      system.abilities.dexterity.mod +
      system.abilities.wisdom.mod +
      system.saves.reflex.bonus;
    system.saves.determination.mod =
      system.abilities.intelligence.mod +
      system.abilities.charisma.mod +
      system.saves.determination.bonus;

    system.isMonster = system.rarity <= 5;

    let baseCastMod = system.specialModifiers.spellcasting
      ? system.specialModifiers.spellcasting.mod
      : 0;
    system.spellMods = {
  ...
  _prepareVehicleData(system) {
    ...
    system.totalWeight = this._getTotalVehicleWeight(
      this.items,
      system.passengers
    );
  ...
  _prepareSkillModifiers(system) {
    ...
    for (let [key, skill] of Object.entries(system.skills)) {
      ...
      skill.rollLabel =
        skill.parentAbilities.length > 1 ? "Save" : "Skill Check";
    ...
    if (typeof system.magic !== "undefined") {
      ...
      for (let [key, skill] of Object.entries(system.magic)) {
        ...
        if (skill.level < 0) skill.level = 0;
        skill.baseMod =
          system.casterLevel + system.abilities[skill.parentAbility].mod;
  ...
  _prepareCalculatedStats(system) {
    ...
    passiveAgility.calculated = Math.max(
      system.abilities.dexterity.score - system.size.mod,
      0
    );
    ...
    passivePerception.value =
      passivePerception.calculated + passivePerception.bonus;
    ...
    initiative.calculated = Math.max(
      system.abilities.dexterity.mod,
      system.abilities.wisdom.mod
    );
    ...
    system.turnLength.actions.value =
      system.turnLength.actions.base + system.turnLength.actions.bonus;
    system.turnLength.reactions.value =
      system.turnLength.reactions.base + system.turnLength.reactions.bonus;

    system.hitPoints.total =
      system.hitPoints.value + system.hitPoints.temporary;
    if (system.hitPoints.value > 0) {
      system.hpPercentage =
        (system.hitPoints.value + system.hitPoints.temporary) /
        system.hitPointTrueMax;
    } else {
    ...
    system.energyPercentage =
      (system.energy.value + system.energy.temporary) / system.energy.max;
   ...
   */
  findResource(name) {
    return Object.values(this.system.additionalResources).find(
  ...
  getClassLevel(clazz) {
    const classObj = this.items.contents.find(
      (i) => i.type == "Class" && i.system.classId == clazz
    );
  ...
  async addSpecialModifier() {
    ...
    update.system.specialModifiers[
      Object.keys(this.system.specialModifiers).length
    ] = {
  ...
  async deleteKnowledge(index) {
    ...
    update.system.knowledges = NewEraUtils.spliceIndexedObject(
      this.system.knowledges,
      index
    );
    ...
    if (this.system.classes) {
      for (const [className, classData] of Object.entries(
        ...
        if (classData.learningExperience) {
          ...
          for (const [level, obj] of Object.entries(
            classData.learningExperience
          )) {
  ...
  async deleteSpecialty(index) {
    ...
    update.system.specialties = NewEraUtils.spliceIndexedObject(
      this.system.specialties,
      index
    );
    ...
    if (this.system.classes) {
      for (const [className, classData] of Object.entries(
        ...
        if (classData.specialtyImprovement) {
          ...
          for (const [level, obj] of Object.entries(
            classData.specialtyImprovement
          )) {
            const prevSelection = parseInt(obj.improvement);
            if (prevSelection == index) {
  ...
  async deleteSpecialModifier(index) {
    await this.update({
      system: {
        specialModifiers: NewEraUtils.spliceIndexedObject(
          this.system.specialModifiers,
          index
        ),
      },
    });
  ...
  async deleteItem(id) {
    ...
    if (item) {
      ...
      await item.delete();
      //Remove the item's ID from any equipment slots it was in
      const update = {
        system: {
          equipment: {},
        },
      };
      for (const key of Object.keys(this.system.equipment)) {
        if (this.system.equipment[key] == id) {
          console.log(`Removing deleted item ${id} from equipment slot ${key}`);
          update.system.equipment[key] = "";
        }
      }
      await this.update(update);
    }
  ...
  async addResource(resource = null) {
    ...
    update.system.additionalResources[
      Object.keys(this.system.additionalResources).length
    ] = resource || {
      name: "New Resource",
      value: 0,
      max: 0,
  ...
  async updateResourceByName(name, resourceUpdate) {
    const index = Object.keys(this.system.additionalResources).find(
      (r) =>
        this.system.additionalResources[r].name.toLowerCase() ==
        name.toLowerCase()
    );
    if (index !== undefined) {
      const update = {
  ...
  async deleteResource(index) {
    const update = {
      system: {
        additionalResources: NewEraUtils.spliceIndexedObject(
          this.system.additionalResources,
          index
        ),
      },
    };
  ...
  async moveItem(id, source, destination) {
    ...
    if (
      ...
      (destination == "leftHand" || destination == "rightHand")
    ) {
      console.log("Checking for a two-handed item...");
      if (
        item.system.handedness == "2H" ||
        (item.system.handedness == "1.5H" &&
          destination == "rightHand" &&
          !system.forceOneHanded)
      ) {
  ...
  async transferItem(recipient, item, targetSlot, quantity = 1) {
    console.log(
      `Entering transferItem ${this.id}->${recipient.id} item=${item.id} x${quantity}`
    );
    ...
    let stacked = false;
    //For stackable items
    if (item.typeIs(NewEraItem.Types.STACKABLE) && !item.system.enchanted) {
      //Enchanted items can differ even when they have the same object ID, so avoid stacking them
      ...
      if (item.system.casperObjectId) {
        if (targetSlot == NewEraActor.EquipmentSlots.BACKPACK) {
          existingItem = recipient.items.find(
            (i) =>
              i.canStackWith(item) &&
              recipient.findItemLocation(i) ==
                NewEraActor.EquipmentSlots.BACKPACK
          );
        } else {
      ...
      if (existingItem) {
        console.log(
          `Found existing item ${existingItem.id} to stack ${quantity} of ${item.name} with.`
        );
        ...
        console.log(
          `No existing item found or casperObjectId is not set. Creating a new item.`
        );
      ...
      console.log(
        `Item is not stackable or is enchanted. Creating a new item.`
      );
    ...
    if (recipient.typeIs(NewEraActor.Types.ANIMATE)) {
      const frameImg =
        "systems/newera-sol366/resources/" +
        (sourceSlot == "backpack" || targetSlot == "backpack"
          ? "ac_3frame.png"
          : "ac_1frame.png");
        ...
        if (this.typeIs(NewEraActor.Types.CHARACTER) && !this.system.defeated) {
          if (item.typeIs(NewEraItem.Types.STACKABLE)) {
            ...
            this.actionMessage(
              ...
              frameImg,
              "{NAME} gave {d} {0} to {1}.",
              item.type == "Phone" ? "phone" : item.name,
              recipient.name
            );
          }
        } else {
          recipient.actionMessage(
            item.img,
            this.img,
            "{NAME} takes the {0}.",
            item.name
  ...
  async splitStack(item, quantity) {
    if (
      !item.typeIs(NewEraItem.Types.STACKABLE) ||
  ...
  actionMessage(baseImage, overlayImage, template, ...args) {
    ...
    html += "</div>";
    html += `<p class="chat-narration">${this.getNarration(
      template,
      ...args
    )}</p>`;
    ChatMessage.create({
  ...
  async takeDamage(
    ...
    const hasLifePoints =
      this.type == "Player Character" || this.type == "Non-Player Character";
    const damageType = dmgType
      ? NEWERA.damageTypes[dmgType]
      : NEWERA.damageTypes.default;
    ...
    const dmg =
      ...
      (damageType.ignoreNatural
        ? 0
        : hasLifePoints
        ? system.armor.natural
        : system.armor.bonus) -
      (!damageType.ignoreEquipped && hitShield && shield
        ? shield.system.shieldRating
        : 0); //Creatures don't have armor.natural - only bonus
    //console.log(`taking damage dmg=${dmg} amount=${amount} equipped=${system.armor.equipped} natural=${system.armor.natural}`);
    if (dmg > 0) {
      this.actionMessage(
        this.img,
        `systems/newera-sol366/resources/dt_${damageType.label.toLowerCase()}.png`,
        "{NAME} takes {0} {1} damage!",
        dmg,
        damageType.label
      );
      if (injury && this.type != "Creature") {
        if (system.hitPoints.max > 1) {
          ...
          if (!system.injured) {
            this.actionMessage(
              this.img,
              `${NEWERA.images}/hand-bandage.png`,
              "{NAME} is injured!"
            );
          }
        } else {
      ...
      if (hasLifePoints) {
        ...
        if (system.hitPoints.temporary > 0) {
          ...
          update.system.hitPoints.temporary = Math.max(
            system.hitPoints.temporary - dmg,
            0
          );
        }
        if (dmgAfterTemporary > 0) {
          if (system.hitPoints.value > 0) {
            if (system.hitPoints.value <= dmgAfterTemporary) {
              ...
              update.system.lifePoints.value =
                system.lifePoints.value - (dmgAfterTemporary - prevHp);
              this.actionMessage(
                this.img,
                `${NEWERA.images}/se_unconscious.png`,
                "{NAME} is down!"
              );
              dying = true;
            } else {
              update.system.hitPoints.value =
                system.hitPoints.value - dmgAfterTemporary;
            }
          } else {
            update.system.lifePoints.value =
              system.lifePoints.value - dmgAfterTemporary;
            if (update.system.lifePoints.value <= 0) {
              update.system.lifePoints.value = 0;
              this.actionMessage(
                `${NEWERA.images}/tombstone.png`,
                this.img,
                "{NAME} is dead!"
              );
              ded = true;
        ...
        if (update.system.hitPoints.value <= 0) {
          ...
          ui.notifications.info(
            `${this.name} has ${system.hitPoints.value - dmg}/${
      ...
      this.actionMessage(
        null,
        null,
        "{NAME} took {0} damage, but it was absorbed by {d} armor!",
        amount
      );
    } else {
  ...
  async heal(amount, overheal = false, recovery = false) {
    ...
    if (newHp > max) {
      ...
      if (overheal) {
        ...
        this.actionMessage(
          this.img,
          `${NEWERA.images}/hp-hot.png`,
          "{NAME} recovers {0} hit points and gains {1} temporary hit points.",
          gained,
          excess
        );
      } else if (system.lifePoints.value < system.lifePoints.max) {
        const newLp = Math.min(
          system.lifePoints.value + excess,
        ...
        this.actionMessage(
          this.img,
          `${NEWERA.images}/hp-hot.png`,
          "{NAME} recovers {0} hit points and {1} life points.",
          gained,
          gainedLp
        );
      } else if (gained > 0) {
    ...
    console.log(
      `HEAL A=${amount} PREV=${prevHp} NEW=${newHp} MAX=${max} G=${gained}`
    );
    console.log(update);
    await this.update(update);
  }
  ...
  async increaseMaxHp(clazz, roll) {
    ...
    if (roll) {
      const r = new Roll(
        `${ClassInfo.hitPointIncrements[className].roll}+@abilities.constitution.mod`,
        this.getRollData()
      );
      await r.evaluate();
      r.toMessage({
  ...
  async rollInitialHP() {
    const system = this.system;
    const initHpRoll = new Roll(
      `${system.initialHitPoints.dieCount}d${system.initialHitPoints.dieSize} + ${system.initialHitPoints.modifier}`,
      this.getRollData()
    );
  ...
  async rest(hours, extraRestful) {
    ...
    */
    ...
    if (gained.energy) {
      this.actionMessage(
        this.img,
        `${NEWERA.images}/ac_restful.png`,
        "{NAME} rests for {0} hours. {S} recover{s|} {1} hit points and {2} energy.",
        hours,
        gained.hitPoints,
        gained.energy
      );
      ...
      this.actionMessage(
        this.img,
        `${NEWERA.images}/ac_restful.png`,
        "{NAME} rests for {0} hours and recovers {1} hit points.",
        hours,
        gained.hitPoints
      );
    }
  ...
  async improveSkill(key, type) {
    ...
    const skillName =
      type == "knowledges"
        ? `Knowledge: ${system.knowledges[key].subject}`
        : game.i18n.localize(`newera.skill.${key}.name`);
    if (currentLevel >= 10) {
      ui.notifications.error(
        `Your ${skillName} skill is already at the maximum level of 10.`
      );
      return;
    }
    ...
    if (cost > system.characterPoints.cpa) {
      ui.notifications.error(
        `Increasing a skill to ${
          currentLevel + 1
        } costs ${cost} character points. You don't have enough.`
      );
      return;
    }
    if (system.level < reqLevel) {
      ui.notifications.error(
        `You must be overall level ${reqLevel} or higher to increase a skill to ${
          currentLevel + 1
        }.`
      );
      return;
    ...
    ui.notifications.info(
      `You spent ${cost} CP to increase your ${skillName} skill to ${
        currentLevel + 1
      }.`
    );
  }
  ...
  async sustain(energyPool = undefined) {
    if (energyPool === undefined) {
      if (
        this.type == "Player Character" ||
        this.type == "Non-Player Character"
      ) {
    ...
    this.actionMessage(
      ...
      spell.img,
      `{NAME} sustains {0}.`,
      NewEraUtils.formatSpellName(spell, ampFactor)
    );
  }
  ...
  async cast(
    ...
    const spellSkillLevel =
      spellSkill == "genericCast"
        ? this.system.casterLevel
        : this.system.magic[spellSkill].level;
    const difficulty = noSkillCheck
      ? 0
      : level <= spellSkillLevel
      ? 0
      : 5 + (level - spellSkillLevel) * 5;
    const energyCost = spell.system.energyCost * ampFactor;
      ...
      if (!stepResults.success) {
        if (energyPool) {
          await energyPool.use(
            stepResults.energyUsed,
            new CharacterEnergyPool(this)
          );
        }
        return false;
      } else {
    ...
    switch (spell.spellRollMode) {
      ...
      case "melee": //This mode is not used as of S366 v1.3 but might be in the future
        alwaysRoll = true;
        attackMod =
          spell.system.castType == "G"
            ? "@skills.two-handed.mod"
            : "@skills.one-handed.mod";
        rollPrefix =
          spell.system.castType == "G"
            ? "Two-Handed Spell Attack -"
            : "One-Handed Spell Attack -";
        break;
      ...
      case "wildMagic": //Same if the spell is crafted and is anything less than perfected, as there's a possibility of wild magic on a low roll
        ...
        rollPrefix = "Cast";
        rollSuffix = ` (${
          NEWERA.spellcraftSuffixes[spell.system.refinementLevel]
        }) `;
        attackMod = "0";
        break;
      default:
    ...
    if (this.type == "Creature") {
      ...
      const castRoll = new Roll(
        `d20 + @abilities.${spellAttr}.mod + @special.${spell.system.specialty} + @special.spellcasting + ${circumstanceModifier}`,
        this.getRollData()
      );
      ...
      const castRoll = new Roll(
        `d20 + ${
          spellSkill == "genericCast"
            ? `@casterLevel`
            : `@magic.${spellSkill}.mod`
        } + @specialty.partial.${spell.system.specialty} + ${attackMod} + ${
          spell.spellcraftModifier
      ...
      castRoll.toMessage({
        speaker: ChatMessage.getSpeaker({ actor: this }),
        flavor: `${rollPrefix} ${NewEraUtils.formatSpellName(
          spell,
          ampFactor
        )}${rollSuffix}${difficulty > 0 ? `(Difficulty ${difficulty})` : ""}`,
      });
    }
    ...
    if (spell.system.castType == "F" && successful) {
      let existingSustain = this.effects.find((e) =>
        e.label.includes("Sustaining")
      );
      if (existingSustain) {
      ...
      await this.createEmbeddedDocuments("ActiveEffect", [
        {
          ...
          ${NewEraUtils.amplifyAndFormatDescription(
  ...
  async castComplexComponents(enchantment, ampFactor = 1) {
    ...
    for (const [stepNo, component] of Object.entries(
      enchantment.system.components
    )) {
      energyConsumed +=
        component.energyCost * (component.scales ? ampFactor : 1);
      const school = NEWERA.schoolOfMagicNames[component.check];
      const form = NEWERA.schoolToFormMapping[component.check];
      const spellSkillLevel =
        form == "genericCast"
          ? this.system.casterLevel
          : this.system.magic[form].level;
      const level = component.level * (component.scales ? ampFactor : 1);
      const difficulty =
        level <= spellSkillLevel ? 0 : 5 + (level - spellSkillLevel) * 5;
      if (difficulty > 0) {
        const castRoll = new Roll(
          `d20 + ${
            form == "genericCast" ? `@casterLevel` : `@magic.${form}.mod`
          } + @specialty.partial.${school}`,
          this.getRollData()
        );
        await castRoll.evaluate();
        ...
        console.log(
          `Skipped rolling for step ${
            parseInt(stepNo) + 1
          } because difficulty is 0`
        );
      }
    }
    return {
  ...
  async stopSustaining() {
    const sustainEffect = this.effects.find((e) =>
      e.label.includes("Sustaining: ")
    );
    if (sustainEffect) {
  ...
  hasConsumable(name, qty) {
    const totalQty = this.items
      .filter(
        (i) => i.name == name && i.type == "Item" && i.system.equipSlot == "C"
      )
  ...
  hasMaterialsFor(spell, ampFactor) {
    if (
      spell.system.keywords.includes("Material") ||
  ...
  async consumeMaterials(enchantment, ampFactor, useAlchemistsPouch) {
    if (useAlchemistsPouch) {
      if (!this.hasResource("Alchemist's Pouch", 1)) {
        ui.notifications.warm(
          "You've used up your Alchemist's Pouch for the day."
        );
        return false;
      } else {
    ...
    for (const material of Object.values(enchantment.system.materialCosts)) {
      ...
      const qtyOfMaterial =
        material.quantity * (material.scales ? ampFactor : 1);
      const used = await this.useConsumable(material.name, qtyOfMaterial);
      if (!used) {
        if (material.unique) {
          //This exception case handles specific materials i.e. 'A piece of someone's DNA' that will not be found by name in inventory
          ui.notifications.warn(
            `This spell requires ${material.name}. Make sure you supply the right materials!`
          );
        } else {
          ui.notifications.error(`Couldn't find all materials!`);
          console.error(
            `Failed to locate enough of ${material.name} - was hasMaterialsFor checked before casting?`
          );
          return false;
        }
      }
  ...
  async usePotion(potion, qty) {
    ...
    if (game.settings.get("newera-sol366", "giveEmptyPotionBottles")) {
      ...
      const bottles = this.items.find(
        (i) =>
          i.type == "Item" &&
          i.system.casperObjectId == NEWERA.EMPTY_POTION_BOTTLE_ID
      );
      if (bottles) {
    ...
    if (game.newera.enforceActionConditions()) {
      ...
      switch (showWhen) {
        ...
        case "twoHanded": //Only used for 1.5H equipment
          available =
            location == "rightHand" &&
            (!equipment.leftHand || !this.system.forceOneHanded);
          break;
        default:
  ...
  async useResource(name, amount) {
    const resource = Object.entries(actor.system.additionalResources).find(
      (r) => r[1].name == name
    );
    if (resource) {
  ...
  hasFeatOrFeature(text) {
    ...
    if (
      this.items.find(
        (i) => i.type == "Feat" && i.name.toLowerCase() == text.toLowerCase()
      )
    ) {
    ...
    Object.entries(this.system.classes).forEach(([key, obj]) => {
      if (ClassInfo.features[key]) {
        const feature = ClassInfo.features[key].find(
          (f) => f.name && f.name.toLowerCase() == text.toLowerCase()
        );
        if (feature) {
  ...
  async lockUnlockContainer() {
    ...
    if (ownership == 3) {
      ui.notifications.info(
        `${this.name} is now UNLOCKED. Players can open and change the token and take items.`
      );
      this.items.forEach((item) => {
        if (
          item.typeIs(NewEraItem.Types.STACKABLE) &&
  ...
  async updateItems() {
    ui.notifications.warn(
      `Update initiated. Please wait a few seconds before doing anything else with ${this.name}.`
    );
    console.log(`Initiating item update for ${this.name}`);
    const spellsEnchantments = await game.packs
      .get("newera-sol366.spells")
      .getDocuments();
    const equipment = await game.packs
      .get("newera-sol366.equipment")
      .getDocuments();
    const consumables = await game.packs
      .get("newera-sol366.consumables")
      .getDocuments();
    const feats = await game.packs.get("newera-sol366.feats").getDocuments();
    const weapons = await game.packs
      .get("newera-sol366.weapons")
      .getDocuments();
    const artifacts = await game.packs
      .get("newera-sol366.potionsArtifacts")
      .getDocuments();
    const wearables = await game.packs
      .get("newera-sol366.wearables")
      .getDocuments();
    const actions = await game.packs
      .get("newera-sol366.actions")
      .getDocuments();

    for (const item of this.items) {
      console.log(
        `Checking for update: ${item.name} [${item.system.casperObjectId}]`
      );
      if (
        item.typeIs(NewEraItem.Types.MAGIC) &&
        item.system.casperObjectId &&
        item.system.rarity > 0
      ) {
        const fromComp = spellsEnchantments.find(
          (i) =>
            i.type == item.type &&
            i.system.casperObjectId == item.system.casperObjectId
        );
        if (fromComp) {
          await item.update({
            name: fromComp.name,
        ...
        item.typeIs(NewEraItem.Types.FEAT) &&
        item.system.casperObjectId
      ) {
        const fromComp = feats.find(
          (i) => i.system.casperObjectId == item.system.casperObjectId
        );
        if (fromComp) {
          await item.update({
            name: fromComp.name,
        ...
        item.typeIs(NewEraItem.Types.BASIC_ITEM) &&
        item.system.casperObjectId
      ) {
        const fromComp =
          equipment.find(
        ...
        item.typeIs(NewEraItem.Types.ARMOR_TABLE) &&
        item.system.casperObjectId
      ) {
        const fromComp =
          equipment.find(
        ...
        if (fromComp) {
          ...
          console.log(
            `${item.name} Not Updated - No Compendium Match (armor/shield)`
          );
        }
      } else if (
        item.typeIs(NewEraItem.Types.POTION) &&
        item.system.casperObjectId
      ) {
        const fromComp = artifacts.find(
          (i) =>
            item.typeIs(NewEraItem.Types.POTION) &&
            i.system.casperObjectId == item.system.casperObjectId
        );
        if (fromComp) {
          ...
          console.log(
            `${item.name} Not Updated - No Compendium Match (potion)`
          );
        }
      } else if (
        item.typeIs(NewEraItem.Types.ACTION) &&
        item.system.casperObjectId
      ) {
        const fromComp = actions.find(
          (i) => i.system.casperObjectId == item.system.casperObjectId
        );
        if (fromComp) {
          await item.update({
          ...
          console.log(
            `${item.name} Not Updated - No Compendium Match (action)`
          );
        }
      } else {
        console.log(
          `${item.name} Not Updated - Not a supported type or not from a compendium`
        );
      }
  ...
  findAmmoFor(item) {
    ...
    if (priority == "strongest") {
      for (const id of compatibleAmmoIds) {
        const ammo = this.items.contents.find(
          (i) => i.type == "Item" && i.system.casperObjectId
        );
        if (ammo) {
          return ammo;
        }
      ...
      const ammo = this.items.contents.filter(
        (i) =>
          i.type == "Item" &&
          compatibleAmmoIds.includes(i.system.casperObjectId)
      );
      if (ammo.length == 0) {
        return null;
      ...
      for (let i = 0; i < this.system.wornItemSlots; i++) {
        ...
        if (this.system.equipment[slot]) {
          ...
          if (
            item &&
            item.type == "Item" &&
            compatibleAmmoIds.includes(item.system.casperObjectId)
          ) {
            return item;
          }
        }
  ...
  hasAmmoFor(item, minimum = 1) {
    const compatibleAmmoIds = NEWERA.compatibleAmmoIds[item.system.ammo.type];
    return this.items.contents.some(
      (i) =>
        i.type == "Item" &&
        compatibleAmmoIds.includes(i.system.casperObjectId) &&
        i.system.quantity >= minimum
    );
  }
  ...
  hasFreeHands(number) {
    if (number > 2) {
      ui.notifications.error(
        "Please contact the Curse Recovery Association at (+29) 049 003 001."
      );
      return false;
    }
    if (game.newera.enforceActionConditions()) {
      if (number == 2) {
        return (
          this.system.equipment.leftHand == "" &&
          this.system.equipment.rightHand == ""
        );
      } else if (number == 1) {
        return !(
          (this.system.equipment.leftHand && this.system.equipment.rightHand) ||
          (this.system.equipment.rightHand &&
            this.items.get(this.system.equipment.rightHand).system.handedness ==
              "2H") ||
          (this.system.equipment.rightHand &&
            this.items.get(this.system.equipment.rightHand).system.handedness ==
              "1.5H" &&
            !this.system.forceOneHanded)
        );
      } else {
        return true;
      }
  ...
  async levelUp(clazz) {
    ...
    if (game.settings.get("newera-sol366", "autoLevelUp")) {
      console.log(
        `[ALU] Starting auto-level up for ${this.name}:${className} ${fromLevel} -> ${toLevel}`
      );
      const archetypeData = ClassInfo.getArchetypeData(this);
      const featuresToUnlock = ClassInfo.features[className].filter(
        (feature) =>
          feature.level > fromLevel &&
          feature.level <= toLevel &&
          (!feature.archetype ||
            archetypeData.hasOwnProperty(feature.archetype)) //This will not catch retroactive archetype features or those that unlock at the same time as the archetype selection, but unlockArchetypeFeatures will
      );
      for (const feature of featuresToUnlock) {
        if (feature.unlocksCoreFeature) {
          //Run this BEFORE the onUnlock handlers, as they may depend on data set by the core feature template!
          if (this.system.coreFeatures[feature.unlocksCoreFeature]) {
            console.log(
              `[ALU] Skipped core feature ${feature.unlocksCoreFeature} because it already has data!`
            );
          } else {
            const coreFeatureData =
              NEWERA.coreFeatureInitData[feature.unlocksCoreFeature];
            await this.update({
              system: {
                coreFeatures: {
            ...
            console.log(
              `[ALU] Activated core feature ${feature.unlocksCoreFeature}`
            );
            ui.notifications.info(
              `You gained the ${coreFeatureData.label} feature!`
            );
          }
        }
        ...
        if (
          ...
          feature.spellStudies.some((s) => typeof s.showWhen != "function")
        ) {
          //Don't trigger this notification for conditional spell studies
          ui.notifications.info(
            `You can learn new spells at this level! Access the Spell Study Guide from the Class tab.`
          );
        }
      }
      const featuresToUpdate = ClassInfo.features[className].filter(
        (feature) =>
          feature.level <= toLevel &&
          feature.tableValues &&
          (!feature.archetype ||
        ...
        feature.tableValues.forEach(async (tv) => {
          if (typeof tv.onUpdate == "function") {
            ...
            if (toVal > fromVal) {
              ...
              console.log(
                `[ALU] Updated Table value for ${feature.name}:${tv.label} from ${fromVal} to ${toVal}`
              );
            } else {
              console.log(
                `[ALU] Table value for ${feature.name}:${tv.label} is still ${fromVal}.`
              );
            }
          } else {
            console.log(
              `[ALU] Table value for ${feature.name}:${tv.label} has no onUpdate function.`
            );
          }
        });
      }
      console.log(
        `[ALU] Auto-Level Up for ${this.name}:${className} ${fromLevel} -> ${toLevel} complete.`
      );
    } else {
      console.log(`[ALU] Auto-Level Up is disabled.`);
    }
    Actions.showHpIncreaseDialog(
      this,
      clazz,
      ClassInfo.hitPointIncrements[className]
    ); //Passing the HP increment here because importing ClassInfo in Actions creates a circular dependency
  }

  async unlockArchetypeFeatures(className, archetype, level) {
    console.log(
      `[ALU] Unlocking archetype features for ${this.name} ${className}:${archetype} level ${level}`
    );
    const archetypeFeatures = ClassInfo.features[className].filter(
      (feature) =>
    ...
    if (to) {
      if (this.system.skills[to]) {
        if (this.system.skills[to].natural) {
          ...
          ui.notifications.info(
            `You're now a natural in ${NewEraUtils.keyToTitle(to)}!`
          );
        }
        ...
        if (this.system.magic[to].natural) {
          ...
          ui.notifications.info(
            `You're now a natural in ${NewEraUtils.keyToTitle(to)} magic!`
          );
        }
      } else {
        ui.notifications.error(`Error: Unable to locate skill key: ${from}`);
  ...
  async setSkillBoost(from, to, levelIncrease = false) {
    ...
    if (from) {
      if (this.system.skills[from]) {
        ...
        //This function and setNaturalSkill might need to support knowledges but there is no need yet.
        ui.notifications.error(`Error: Unable to locate skill key: ${from}`);
      }
    }
    if (to) {
      if (this.system.skills[to]) {
        if (levelIncrease && this.system.skills[to].level == 10) {
          ui.notifications.warn(
            `Your ${NewEraUtils.keyToTitle(to)} skill is already at max level!`
          );
        } else {
          update.skills[to] = {
          ...
          if (levelIncrease) {
            ui.notifications.info(
              `Your ${NewEraUtils.keyToTitle(to)} skill increased to ${
                update.skills[to][field]
              }!`
            );
          } else {
            ui.notifications.info(
              `Your ${NewEraUtils.keyToTitle(to)} skill bonus is now ${
                update.skills[to][field]
              }.`
            );
          }
        ...
        if (levelIncrease && this.system.magic[to].level == 10) {
          ui.notifications.warn(
            `Your ${NewEraUtils.keyToTitle(
              to
            )} magic skill is already at max level!`
          );
        } else {
          update.magic[to] = {
          ...
          if (levelIncrease) {
            ui.notifications.info(
              `Your ${NewEraUtils.keyToTitle(to)} magic skill increased to ${
                update.magic[to][field]
              }!`
            );
          } else {
            ui.notifications.info(
              `Your ${NewEraUtils.keyToTitle(to)} magic skill bonus is now ${
                update.magic[to][field]
              }.`
            );
          }
  ...
  async setSpecialtyFeature(
    ...
    const defaultParent =
      parentSkill ||
      NEWERA.specialtyDefaultParents[subject] ||
      NEWERA.specialtyDefaultParents[to];
    const update = {
      specialties: {},
    };
    if (from) {
      const fromIndex = Object.keys(this.system.specialties).find(
        (spec) =>
          this.system.specialties[spec].subject == NewEraUtils.keyToTitle(from)
      );
    ...
    if (to) {
      ...
      const toIndex = Object.keys(this.system.specialties).find(
        (spec) => this.system.specialties[spec].subject == title
      );
      if (toIndex !== undefined) {
        if (allowStacking) {
          if (this.system.specialties[toIndex].level < 3) {
            ...
            ui.notifications.info(
              `Your ${title} specialty increased to ${update.specialties[toIndex].level}!`
            );
          } else {
            ui.notifications.warn(
              `Your ${title} specialty is already at max level! Specialties can't exceed level 3.`
            );
          }
        } else {
          ui.notifications.warn(
            `You already have a specialty in ${title}. You can't use this feature to increase its level.`
          );
        }
      } else {
  ...
  async setCasterLevel(from, to, setMaxEnergy = true) {
    const energyChange = setMaxEnergy
      ? NEWERA.baseEnergyMaximums[to] - NEWERA.baseEnergyMaximums[from || 0]
      : 0;
    await this.update({
      system: {
        casterLevel: to,
  ...
  async setAbilityScoreImprovement(from, to) {
    ...
    if (from) {
      ...
      if (
        from == "constitution" &&
        NEWERA.abilityScoreModifiers[update.abilities.constitution.score] <
          NEWERA.abilityScoreModifiers[this.system.abilities.constitution.score]
      ) {
        update.hitPoints = {
    ...
    if (to) {
      if (this.system.abilities[to].score >= 20 && this.system.level < 15) {
        ui.notifications.warn(
          `You can't increase an ability score beyond 20 until you reach level 15.`
        );
        ...
        ui.notifications.info(
          `Your ${to} increased to ${update.abilities[to].score}!`
        );
        ...
        if (
          ...
          NEWERA.abilityScoreModifiers[update.abilities.constitution.score] >
            NEWERA.abilityScoreModifiers[
              this.system.abilities.constitution.score
            ]
        ) {
          update.hitPoints = {
            max: this.system.hitPoints.max + 1, //This method only ever changes the score by 1
  ...
  async setLearningExperience(from, to) {
    ...
    if (to) {
      ...
      ui.notifications.info(
        `Your ${knowledge.subject} knowledge increased to ${
          knowledge.level + 1
        }!`
      );
    }
  ...
  async setSpecialtyImprovement(from, to) {
    ...
    if (to) {
      ...
      ui.notifications.info(
        `Your ${specialty.subject} specialty increased to ${
          specialty.level + 1
        }!`
      );
    }
    ...
    if (to) {
      if (this.system.skills[to]) {
        if (this.system.skills[to].level < 10) {
          ui.notifications.warn(
            `You must reach skill level 10 before you can become a grand master!`
          );
        } else {
          update.skills[to] = {
            grandMaster: true,
          ...
          ui.notifications.info(
            `${this.name} is now a grand master in ${NewEraUtils.keyToTitle(
              to
            )}!`
          );
        }
        ...
        if (this.system.magic[to].level < 10) {
          ui.notifications.warn(
            `You must reach magic level 10 before you can become a grand master!`
          );
        } else {
          update.magic[to] = {
            grandMaster: true,
