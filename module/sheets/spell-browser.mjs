...
export class SpellBrowser extends ActorSheet {
    ...
    async _searchSpells(criteria){
        let results = [];
        this.compendium = await game.packs.get("newera-sol366.spells").getDocuments().filter(doc => doc.typeIs(NewEraItem.Types.MAGIC)); //As of v1.3, alchemy recipes now live in the spells compendium. We only want the spell browser to deal with spells and enchantments
