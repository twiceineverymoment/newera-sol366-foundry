import {
  ...
  toggleEffect,
  getEffectDuration,
  findStandardActiveEffectByName,
...
export class NewEraActorSheet extends ActorSheet {
  ...
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, {
      ...
      tabs: [
        ...
        {
          navSelector: ".spell-tabs",
          contentSelector: ".spell-table",
          initial: "all",
        },
      ],
    });
  ...
  getData() {
    ...
    if (this.actor.typeIs(NewEraActor.Types.PC)) {
      ...
      const [custom, standard] = NewEraUtils.splitIndexedObject(
        context.system.additionalResources,
        (r) => r.custom
      );
      context.resources = {
        standard,
  ...
  _prepareCharacterData(context) {
    ...
    for (let [key, obj] of Object.entries(context.system.specialties)) {
      obj.label =
        obj.subject +
        " (" +
        game.i18n.localize("newera.skill." + obj.defaultParent + ".abbr") +
        ")";
    }
        ...
        if (feature.level <= clazz.system.level) {
          if (feature.archetype) {
            ...
            let showFeature =
              ...
              unlockThreshold &&
              (unlockThreshold <= feature.level || feature.retroactiveUnlock);
            if (!showFeature) {
              continue;
            }
          }
          feature.className = feature.archetype
            ? NEWERA.classes[clazz.system.selectedClass].archetypes[
                feature.archetype
              ]
            : clazz.system.selectedClass;
          feature.classImg = feature.archetype
            ? `${NEWERA.images}/${
                className == "researcher" ? "" : className + "_"
              }${feature.archetype}.png`
            : clazz.img;
          ...
          if (feature.common) {
            ...
            if (feature.selections) {
              feature.id = `${feature.className.toLowerCase()}.${
                feature.common
              }.${feature.level}`; //Give common features a unique id so their selections can persist in the data model
            }
          }
          ...
          if (feature.spellStudies) {
            for (let i = 0; i < feature.spellStudies.length; i++) {
              ...
              if (typeof feature.spellStudies[i].showWhen == "function") {
                try {
                  feature.spellStudies[i].show = feature.spellStudies[
                    i
                  ].showWhen(this.actor);
                } catch (err) {
              ...
              feature.spellStudies[i].status =
                feature.spellStudies.length > 1
                  ? `(${i + 1}/${feature.spellStudies.length})`
                  : "";
              feature.spellStudies[i].index = i;
              try {
                const remainingSelections =
                  this.actor.system.classes[
                    clazz.system.selectedClass.toLowerCase()
                  ].spellStudies[feature.level][i.toString()];
                feature.spellStudies[i].remaining =
                  remainingSelections !== undefined
                    ? remainingSelections
                    : feature.spellStudies[i].choose;
                ...
                if (feature.spellStudies[i].complete) {
                  ...
                  feature.spellStudies[
                    i
                  ].status += ` (${remainingSelections} Remaining)`;
                }
              } catch (err) {
      ...
      basic: features.sort((a, b) => {
        if (a.level < b.level) {
          ...
          if (a.className == "Adventurer" && b.className != "Adventurer") {
            ...
            a.className != "Adventurer" &&
            b.className == "Adventurer"
          ) {
            return -1;
          } else {
            return 0;
  ...
  _prepareFeatOptions(context, feats) {
    ...
    for (const feat of feats) {
      if (
        feat.system.casperObjectId &&
        globalFeatData[feat.system.casperObjectId]
      ) {
        for (let tier = 1; tier <= feat.system.currentTier; tier++) {
          if (
            globalFeatData[feat.system.casperObjectId][tier] &&
            globalFeatData[feat.system.casperObjectId][tier].features
          ) {
            for (const feature of globalFeatData[feat.system.casperObjectId][
              tier
            ].features) {
              ...
              if (feature.selections) {
                ...
                Object.entries(feature.selections).forEach(
                  ([key, selection]) => {
                    //Populate the options for dynamic selections
                    if (typeof selection.dynamicOptions == "function") {
                      selection.options = selection.dynamicOptions(this.actor);
                    ...
                    try {
                      const currentValue = keys.reduce(
                        (acc, key) => acc[key],
                        this.actor.system.featSelections
                      );
                      selection.currentValue = currentValue;
              ...
              if (feature.selections || feature.spellStudies) {
                //Features which have data defined purely for unlock triggers or prerequisites have no need to be rendered on the sheet
  ...
  _prepareItems(context) {
    ...
    if (equipment.rightHand) {
      ...
      if (itemInMainHand) {
        if (
          itemInMainHand.system.handedness == "2H" ||
          (itemInMainHand.system.handedness == "1.5H" &&
            !context.system.forceOneHanded)
        ) {
    ...
    for (let i of context.items) {
      ...
      switch (i.type) {
        ...
        case "Item":
          ...
          i.showMergeStacks =
            i.system.casperObjectId &&
            !i.system.enchanted &&
            context.items.some((j) => NewEraItem.canStack(i, j));
        case "Melee Weapon":
        case "Ranged Weapon":
        case "Armor":
        case "Shield":
        case "Phone":
          ...
          for (const [slot, id] of Object.entries(equipment)) {
            if (id == i._id) {
              //This should return false for the worn array, so no need for a type check here (probably)
              inBackpack = false;
              if (slot.includes("worn")) {
                const slotNum = parseInt(slot.substring(4));
                if (slotNum < context.system.wornItemSlots) {
                  //Only populate the slots for which the actor has enough room
                  worn[slotNum] = i;
                } else {
    ...
    const actions = {
      ...
      show: {
        ...
        social:
          ...
          actor.type == "Non-Player Character",
        magic:
          (actor.type == "Creature" && system.hasMagic) ||
          system.casterLevel > 0,
        spells:
          actor.typeIs(NewEraActor.Types.CHARACTER) &&
          system.casterLevel > 0 &&
          game.settings.get("newera-sol366", "favoriteSpellActions"),
      },
    };

    if (system.hitPoints.total > 0 || actor.type == "Creature") {
      //If a PC or NPC's HP is 0, remove all actions and show only the death save
      ...
      for (const i of sortedItemIds) {
        ...
        */
          itemAction.itemNotEquipped =
            !this.actor.isItemActionAvailable(itemAction, item) &&
            actor.type != "Creature";
      ...
      if (
        ...
        actions.general.forEach((a) =>
          this._prepareActionContextInfo(a, false)
        );
        actions.exploration.forEach((a) =>
          this._prepareActionContextInfo(a, false)
        );
      ...
      if (actions.show.spells) {
        const spells = this.actor.items.filter(
          (i) => i.type == "Spell" && system.favoriteSpells.includes(i.id)
        );
        spells.forEach((spell) => {
          const actionSheetData = {
            ...
            skill: null,
            specialties: [],
            description: NewEraUtils.amplifyAndFormatDescription(
      ...
      /* Unarmed attack */
      if (
        ((!system.equipment.leftHand || !system.equipment.rightHand) &&
          actor.type != "Creature") ||
        system.hasStandardActions
      ) {
      ...
      if (system.lastAction) {
        actions.selected =
          actions[system.lastAction.category][system.lastAction.index] || false;
      ...
      actions.death = {
        ...
        specialties: [],
        description:
          "While you have 0 HP, you make an endurance save on your turn. If you succeed, you regain 1 HP and wake up, and your turn ends. If you fail, you lose 5 life points.",
        difficulty: 15,
        type: "0",
        rolls: [
          {
  ...
  _prepareActionContextInfo(action, isCustom) {
    ...
    const skillInfo = isCustom
      ? ""
      : action.ability
      ? ` - ${action.ability.charAt(0).toUpperCase()}${action.ability.slice(
          1
        )} check`
      : action.skill
      ? ` - ${action.skill.charAt(0).toUpperCase()}${action.skill.slice(
          1
        )} check`
      : "";
    action.typeDescription = `${
      actionTypes[action.actionType] || "Generic Action"
    }${skillInfo}`;
    ...
    if (action.itemNotEquipped) {
      ...
      action.disabled =
        ["E", "S", "D"].includes(action.actionType) &&
        game.combat &&
        game.combat.active &&
        game.combat.round > 0
          ? "You can't do this during combat."
          : false;
    }

    const rolls = action.rolls
      ? typeof action.rolls == "Array"
        ? action.rolls
  ...
  activateListeners(html) {
    ...
    html.find("select.auto-value").each((i, element) => {
      const dataField =
        $(element).attr("name") || $(element).data("indirectName"); // Using data-indirect-name prevents the select from being picked up by Foundry's built-in updates, in cases where the manual listener performs an update (to avoid updating twice)
    ...
    html.find(".occupant-display").click((ev) => {
      const actorId = $(ev.currentTarget)
        .parents(".vehicle-occupant")
        .data("actorId");
    ...
    html.find(".occupant-delete").click((ev) => {
      ...
      ui.notifications.info(
        `${actor.name} is no longer a passenger in ${this.actor.name}.`
      );
    });

    //Ability Score Point Buy
    if (
      system.level <= game.settings.get("newera-sol366", "startingLevel") &&
      game.settings.get("newera-sol366", "characterCreation") &&
      this.actor.typeIs(NewEraActor.Types.PC)
    ) {
      ...
      if (system.abilityScorePointBuy) {
        if (system.abilityScorePointBuy.outOfRange) {
          ...
          html
            .find("#ability-points-msg")
            .html("Starting ability scores must be between 6 and 15");
          ...
          html
            ...
            .html(
              "<b>" +
                system.abilityScorePointBuy.remaining +
                "</b> points remaining"
            );
        } else {
          html.find("#ability-points-msg").css("color", "red");
          html
            ...
            .html(
              "<b>" +
                Math.abs(system.abilityScorePointBuy.remaining) +
                "</b> points over"
            );
        }
      }
    ...
    if (this.actor.typeIs(NewEraActor.Types.ANIMATE)) {
      if (system.energy.max == 0) {
        html
          .find("#energy-icon")
          .attr("src", "systems/newera-sol366/resources/energy.png");
        ...
        html
          .find("#energy-icon")
          .attr("src", "systems/newera-sol366/resources/energy-over.png");
    ...
    if (this.actor.typeIs(NewEraActor.Types.CHARACTER)) {
      html.find(".inventory-entry-magic").each((i, val) => {
        ...
        if (item.typeIs(NewEraItem.Types.MAGIC)) {
          html
            .find(`.spell-action-icons.${id}`)
            .html(NewEraUtils.getSpellActionIcons(item));
        }
      });
      //Monster magic stuff
      ...
      html.find(".inventory-entry-magic").each((i, val) => {
        ...
        html
          .find(`.spell-action-icons.${spellId}`)
          .html(NewEraUtils.getSpellActionIcons(this.actor.items.get(spellId)));
        if (ampFactor == 1) {
    ...
    html.find(".action-icon").click(async (ev) => {
      if ($(ev.currentTarget).data("actionCategory")) {
        //This check prevents the listener from triggering on click of the 'new custom action' button
    ...
    if (game.settings.get("newera-sol366", "autoLevelUp")) {
      html.find(".feature-select").change(async (ev) => {
        ...
        console.log(
          `[ALU] Triggered feature selection change ${className}:${featureSelectionId}.${selectionIndex} ${oldValue} -> ${newValue}`
        );
        try {
          const feature = ClassInfo.features[className].find(
            (feature) =>
              feature.id == featureSelectionId &&
              feature.selections &&
              feature.selections[selectionIndex]
          );
          ...
          ui.notifications.error(
            "Error: Couldn't update data for this selection. See the log for details."
          );
          console.error(
            `[ALU] Failed to locate selection change function! class=${className} id=${featureSelectionId} index=${selectionIndex}`
          );
          console.error(err);
        }
      });
      html.find(".feat-select").change(async (ev) => {
        ...
        console.log(
          `[ALU] Triggered feat selection change ${featId}.${tier}.${selectionIndex} ${oldValue} -> ${newValue}`
        );
        try {
          const featData = ExtendedFeatData.getFeatures(featId, tier);
          const selection = featData.find((f) => f.selections[selectionIndex])
    ...
    html
      .find(".feat-browser")
      .click(() => new FeatBrowser(this.actor).render(true));
    html
      .find(".spell-browser")
      .click(() => new SpellBrowser(this.actor).render(true));
    html.find(".spell-studies").click((ev) => {
      ...
      console.log(
        `[DEBUG] loading spell study guide class=${className} arch=${archetype} level=${level} index=${index}`
      );
      const spellStudiesCriteria = ClassInfo.features[
      ...
      if (spellStudiesCriteria && spellStudiesCriteria[index]) {
        ...
        new SpellBrowser(this.actor, criteria, {
        ...
        ui.notifications.error(
          "Couldn't load spell studies for this level. Please report this to the developers."
        );
      }
    });
    ...
    html.find(".spell-favorite-remove").click(async (ev) => {
      ...
      let favorites = structuredClone(this.actor.system.favoriteSpells).filter(
        (s) => s != li.data("itemId")
      );
      await this.actor.update({
        system: {
    ...
    html.find(".deleteResource").click((ev) => {
      NewEraUtils.confirm(this.actor, ev, (actor, event) =>
        actor.deleteResource($(event.currentTarget).data("resourceIndex"))
      );
    });
    html.find(".deleteKnowledge").click((ev) => {
      NewEraUtils.confirm(this.actor, ev, (actor, event) =>
        actor.deleteKnowledge($(event.currentTarget).data("knowledgeIndex"))
      );
    });
    html.find(".deleteSpecialty").click((ev) => {
      NewEraUtils.confirm(this.actor, ev, (actor, event) =>
        actor.deleteSpecialty($(event.currentTarget).data("specialtyIndex"))
      );
    });
    html.find(".deleteSpecialModifier").click((ev) => {
      NewEraUtils.confirm(this.actor, ev, (actor, event) =>
        actor.deleteSpecialModifier(
          $(event.currentTarget).data("specialModifierIndex")
        )
      );
    });
    ...
    html.find(".newera-draggable-equipment").on("dragstart", (ev) => {
      ...
      if (item.system.rollQuantity) {
        ui.notifications.warn(
          "This item has a variable quantity. You must roll to determine the quantity before you can move it."
        );
        ev.originalEvent.dataTransfer.effectAllowed = "none";
      } else {
        const fromZone = $(ev.currentTarget).parents(
          ".newera-equipment-dropzone"
        );
    ...
    html.find(".newera-equipment-dropzone").on("drop", async (ev) => {
      ...
      console.log(
        `INV DROP ${itemId} ${sourceActor}.${sourceSlot}->${targetActor}.${targetSlot}`
      );
      ...
      if (sourceActor == targetActor) {
        ...
        if (sourceSlot == targetSlot || !sourceSlot) {
          ...
          ui.notifications.error(
            "That item can't be placed in that slot. Try a different location."
          );
        } else if (
          sourceSlot == "backpack" &&
          movedItem.system.stored == true
        ) {
          ui.notifications.error(
            "That item is in storage. You must retrieve it before you can equip it."
          );
        } else {
          const frameImg =
          ...
          if (NewEraUtils.sendEquipmentChangeMessages()) {
            this.actor.actionMessage(
              movedItem.img,
              frameImg,
              "{NAME} {0} {d} {1}!",
              this._getItemActionVerb(sourceSlot, targetSlot),
              movedItem.type == "Phone" ? "phone" : movedItem.name
            );
          }
      ...
      //Moving an item to a different actor
        ...
        if (!origin) {
          ui.notifications.error(
            "Failed to move item: Unable to locate source actor"
          );
          return;
        }
        ...
        if (!movedItem) {
          ui.notifications.error(
            "Failed to move item: The source actor doesn't seem to have that item."
          );
          return;
        }
        if (!this._isItemDroppable(movedItem, targetSlot)) {
          ...
          ui.notifications.error(
            "That item is in storage. You must retrieve it before you can give it to someone else."
          );
        } else {
          let quantity = 1;
    ...
    html.find("button.newera-action-button").click((ev) => {
      ...
      const element = $(ev.currentTarget);
      const actionData = context.actions.selected.rolls.find(
        (r) => (r.label = element.data("actionButton"))
      );
      if (actionData && typeof actionData.callback == "function") {
        actionData.callback(this.actor);
      } else {
        console.error(
          `Missing callback function for action button ${element.data(
            "actionButton"
          )} on ${actions.selected.name}`
        );
      }
    });
  ...
  _onRoll(event) {
    ...
    if (dataset.rollType) {
      ...
      if (dataset.rollType == "spell") {
        const spellDetail = `${
          dataset.ampFactor > 1
            ? ` ${NEWERA.romanNumerals[dataset.ampFactor]}`
            : ""
        }${
          parseInt(dataset.difficulty) > 0
            ? ` (Difficulty ${dataset.difficulty})`
            : ""
        }`;
        console.log(
          `SPELLCASTMSG diff=${dataset.difficulty} ampFactor=${dataset.ampFactor} detail=${spellDetail}`
        );
        dataset.spellCaption = dataset.label + spellDetail;
        if (dataset.difficulty == "0" && dataset.spellType == "cast") {
          this.actor.actionMessage(
            this.actor.img,
            `${NEWERA.images}/${dataset.spellSchool}.png`,
            "{NAME} casts {0}!",
            `${dataset.name}${spellDetail}`
          );
          return;
        }
        ...
        if (this.actor.type == "Creature") {
          ...
          if (dataset.spellType == "attack") {
            totalCastMod +=
              systemData.skills.marksmanship.level +
              systemData.skills.marksmanship.bonus;
          }
        }
        ...
        roll.toMessage({
          speaker: ChatMessage.getSpeaker({ actor: this.actor }),
          flavor: (dataset.rollType = "spell"
            ? dataset.spellCaption
            : dataset.label),
          rollMode: game.settings.get("core", "rollMode"),
        });
  ...
  updateResourceDisplay() {
    ...
    if (system.energy.max == 0) {
      ...
      html
        .find(".resource-energy")
        .classList.remove("resource-energy-no-magic");
    }
  ...
  _getSpellCastDifficulty(spellId, ampFactor) {
    ...
    if (spell.system.school == "CL") {
      //Channeling spells don't have a casting difficulty.
      return null;
    }
  ...
  _isItemDroppable(item, destination) {
    ...
    const droppable =
      slotsAllowed.includes(destination) ||
      destination == "backpack" ||
      (destination.includes("worn") && slotsAllowed.includes("worn")); //The "worn" case is necessary because the slotsAllowed method doesn't explicitly list all 10 worn item slots
    return droppable;
  }
  ...
  _getItemActionVerb(source, destination) {
    if (source == "backpack") {
      if (
        destination == "leftHand" ||
        destination == "rightHand" ||
        destination == "phone" ||
        destination.includes("worn")
      ) {
        return "takes out";
      } else {
        return "puts on";
    ...
    new Dialog({
      ...
      content: () => {
        ...
        <select id="skillSelected">
          ...
          `;
        for (const [i, knowledge] of Object.entries(
  ...
  _prepareInspiration(context) {
    ...
    context.inspiration = {
      enabled:
        game.settings.get("newera-sol366", "inspiration") && difficulty < 3,
      points: {},
      cp: Math.floor(
        this.actor.system.inspiration * this.actor.system.levelGap * 0.1
      ),
    };
  ...
  async _onActorSheetDrop(event) {
    ...
    if (dropData && dropData.type == "Actor" && this.actor.type == "Vehicle") {
      ...
      if (passenger && passenger.typeIs(NewEraActor.Types.ANIMATE)) {
        ...
        ui.notifications.info(
          `${passenger.name} is now a passenger in ${this.actor.name}.`
        );
      }
    }
    if (dropData && dropData.transferAction) {
      ...
      if (dropData.transferAction == "addFeatFromBrowser") {
        const compendium = await game.packs
          .get("newera-sol366.feats")
          .getDocuments();
        const featFromCompendium = compendium.find(
          (feat) => feat.system.casperObjectId == dropData.casperObjectId
        );
        const featFromActor = this.actor.items.find(
          (feat) => feat.system.casperObjectId == dropData.casperObjectId
        );
        ...
        if (featFromCompendium) {
          if (featFromActor) {
            ...
            if (featFromActor.system.maximumTier == -1) {
              ...
              ui.notifications.info(
                `You took another instance of ${featFromActor.name} for ${featFromCompendium.system.base.cost} character points.`
              );
            } else if (featFromActor.system.maximumTier == 1) {
              ui.notifications.error(
                `${this.actor.name} already has ${featFromCompendium.name}.`
              );
            } else {
              if (
                featFromActor.system.currentTier ==
                featFromActor.system.maximumTier
              ) {
                ui.notifications.error(
                  `${this.actor.name} already has the highest available tier of ${featFromCompendium.name}.`
                );
              } else {
                if (
                  featFromActor.characterMeetsFeatPrerequisites(
                    this.actor,
                    featFromActor.system.currentTier + 1
                  )
                ) {
                  ...
                  ui.notifications.info(
                    `You upgraded ${this.actor.name}'s ${
                      featFromCompendium.name
                    } feat to tier ${featFromActor.system.currentTier} for ${
                      featFromActor.system.tiers[
                        featFromActor.system.currentTier
                      ].cost
                    } character points.`
                  );
                  ...
                  ui.notifications.warn(
                    `${
                      this.actor.name
                    } doesn't meet the prerequisites for tier ${
                      featFromActor.system.currentTier + 1
                    } of ${featFromActor.name}.`
                  );
                }
              }
            }
            ...
            //Add new feat
            if (
              featFromCompendium.characterMeetsFeatPrerequisites(this.actor, 1)
            ) {
              const featData = structuredClone(featFromCompendium);
              await Item.create(featData, { parent: this.actor });
              tierUnlocked = 1;
              ui.notifications.info(
                `You took ${featFromCompendium.name} for ${featFromCompendium.system.base.cost} character points.`
              );
            } else {
              ui.notifications.warn(
                `${this.actor.name} doesn't meet the prerequisites for ${featFromCompendium.name}.`
              );
            }
          }
          ...
          if (
            tierUnlocked &&
            game.settings.get("newera-sol366", "autoLevelUp")
          ) {
            console.log(
              `[ALU] Processing unlock triggers for ${featFromCompendium.name} tier ${tierUnlocked}`
            );
            try {
              const features = ExtendedFeatData.getFeatures(
                dropData.casperObjectId,
                tierUnlocked
              );
              for (const feature of features) {
                if (feature.unlocksCoreFeature) {
                  ...
                  if (this.system.coreFeatures[feature.unlocksCoreFeature]) {
                    ...
                    console.log(
                      `[ALU] Activated core feature ${feature.unlocksCoreFeature} from feat ${featFromCompendium.name}`
                    );
                    ui.notifications.info(
          ...
          ui.notifications.error(
            "Couldn't find a feat in the CASPER database matching this item. Please report this to the developers."
          );
        }
        ...
        if (dropData.studies && dropData.studies.className) {
          if (this.actor.id != dropData.actorId) {
            ui.notifications.error(
              `This spell study guide is for a different actor.`
            );
            return;
          }
          if (dropData.remaining == -1) {
            ...
            update.system.classes[dropData.studies.className].spellStudies[
              dropData.studies.level
            ] = {};
            update.system.classes[dropData.studies.className].spellStudies[
              dropData.studies.level
            ][dropData.studies.index] = dropData.remaining - 1;
            await this.actor.update(update);
          } else {
            ui.notifications.warn(
              "You've completed your spell studies for this list. Please close the study guide and choose another one."
            );
            return;
          }
        }
        const compendium = await game.packs
          ...
          .getDocuments();
        const spellFromCompendium = compendium.find(
          (spell) =>
        ...
        if (spellFromCompendium) {
          ...
          this.actor.actionMessage(
            this.actor.img,
            spellFromCompendium.img,
            "{NAME} learned {0}!",
            spellFromCompendium.name
          );
          if (dropData.remaining > 1) {
            ui.notifications.info(
              `You learned ${spellFromCompendium.name}! You can choose ${
                dropData.remaining - 1
              } more spell${
                dropData.remaining > 2 ? "s" : ""
              } from the current study guide.`
            );
          } else if (dropData.remaining == 1) {
            ui.notifications.info(
              `You learned ${spellFromCompendium.name}! You've completed this study guide.`
            );
          } else {
