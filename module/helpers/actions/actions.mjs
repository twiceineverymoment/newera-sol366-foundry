...
export class Actions {
  ...
  static showHpIncreaseDialog(actor, clazz, increment) {
    ...
    new Dialog({
      ...
      content:
        "Choose whether to roll for your hit point increase, or take the average.",
      buttons: {
        roll: {
          ...
          callback: async (html) => {
            ...
            if (hours > 12) {
              ...
              ui.notifications.warn(
                "You can't rest for more than 12 hours at a time."
              );
            }
  ...
  static async sustainCurrentSpell(actor) {
    ...
    if (
      energyRequired &&
      actor.energyPools.filter((p) => !p.depleted).length == 0
    ) {
      ui.notifications.error(
        "Your energy is depleted. You can still continue sustaining the spell if you recover enough energy before the end of your turn."
      );
      return;
    }
    ...
    const spellTitle = NewEraUtils.formatSpellName(
      spell,
      actor.system.sustaining.ampFactor
    );
    let dialog = new Dialog({
      title: `Sustain ${spellTitle} [${actor.name}]`,
      content: `<form class="spell-dialog">
          <div id="energySelect">
            Energy Source: <select id="energyPools">${this._renderPoolOptions(
              actor
            )}</select>
          </div>
          <p>
            <button class="spell-dialog-button" id="cast"><i class="fa-solid fa-hand-sparkles"></i> Sustain</button>
            <button class="spell-dialog-button" id="attack"${
              spell.system.rangedAttack
                ? ""
                : `disabled data-tooltip="This spell isn't a projectile attack."`
            }><i class="fa-solid fa-crosshairs"></i> Attack</button>
            <button class="spell-dialog-button" id="damage"${
              spell.system.damage && spell.system.damage.type
                ? ""
                : `disabled data-tooltip="This spell doesn't deal damage."`
            }><i class="fa-solid fa-heart-crack"></i> Damage</button>
          </p>
        </form>
        `,
      buttons: {
        ...
        html.find("#damage").click(async () => {
          ...
          const formula =
            spell.name == "Lightning Bolt"
              ? NEWERA.lightningBoltDamageRolls[amp]
              : NewEraUtils.amplifyValue(spell.system.damage.amount, amp);
  ...
  static async castSpell(actor, spell, paramAmpFactor = 1, isPrepared = false) {
    if (!actor.canCastSpell(spell)) {
      if (["G", "L", "R"].includes(spell.system.castType)) {
        ui.notifications.warn(
          `${actor.name} needs both hands free to cast ${spell.name}!`
        );
      } else {
        ui.notifications.warn(
          `${actor.name} needs a free hand to cast ${spell.name}!`
        );
      }
      return;
    }
    ...
    if (energyRequired) {
      ...
      if (pools.filter((p) => !p.depleted).length == 0) {
        ui.notifications.error(
          "Your energy is depleted. You can't cast any spells until you drink a potion or rest to recover."
        );
        return;
      }
    }
    ...
    let dialog = new Dialog({
      ...
      content: `<form class="spell-dialog">
          ...
          <div id="energySelect">
            Energy Source: <select id="energyPools">${Actions._renderPoolOptions(
              actor
            )}</select>
          </div>
          <div id="materialCosts"></div>
          <p>
            <button class="spell-dialog-button" id="cast"><i class="fa-solid ${
              castButton.icon
            }"></i> ${castButton.label}</button>
            <button class="spell-dialog-button" id="damage"${
              spell.system.damage && spell.system.damage.type
                ? ""
                : `disabled data-tooltip="This spell doesn't deal damage."`
      ...
      render: (html) => {
        const initialAmpFactor =
          actor.type == "Creature" ? spell.system.ampFactor : paramAmpFactor;
        Actions._renderSpellDetails(
          html,
          spell,
          actor,
          initialAmpFactor,
        ...
        html.find("#amplify-up").click(() => {
          const amp =
            actor.type == "Creature"
              ? spell.system.ampFactor
              : parseInt(html.find("#ampFactor").html());
        ...
        html.find("#damage").click(async () => {
          ...
          const formula =
            spell.name == "Lightning Bolt"
              ? NEWERA.lightningBoltDamageRolls[amp]
              : spell.system.damage.scales
              ? NewEraUtils.amplifyValue(spell.system.damage.amount, amp)
              : spell.system.damage.amount;
          const dmgRoll = new Roll(formula);
          await dmgRoll.evaluate();
          dmgRoll.toMessage({
            speaker: ChatMessage.getSpeaker({ actor: actor }),
  ...
  static _renderPoolOptions(actor) {
    ...
    for (const pool of actor.energyPools) {
      options += `<option value="${pool.id}" ${
        pool.depleted ? "disabled" : ""
      }>${pool.name} (${pool.available}/${pool.max})</option>`;
    }
    return options;
  }
  ...
  static _renderSpellDetails(html, spell, actor, ampFactor, prepared) {
    ...
    const spellSkillLevel =
      spellSkill == "genericCast"
        ? actor.system.casterLevel
        : actor.system.magic[spellSkill].level;
    const difficulty = prepared
      ? 0
    ...
    const thingsAreComplex =
      spell.type == "Enchantment" && spell.system.enchantmentType == "CE";
    const passiveSpellSkill =
      10 +
      (spellSkill == "genericCast"
        ? actor.system.casterLevel
        : actor.system.magic[spellSkill].mod);
    let pctChance = thingsAreComplex
      ? Actions._getComplexPercentage(spell, actor, ampFactor)
      : 55 + (passiveSpellSkill - difficulty) * 5;
    ...
    if (actor.type == "Creature") {
      ...
      html
        .find("#cost")
        .html(`${energyCost} (-${energyCost - availableEnergy})`);
    ...
    if (difficulty == 0) {
      ...
      //Less than 5 here because 0 to -45% is still 5% chance because of the possibility of a nat 20
    ...
    if (thingsAreComplex) {
      ...
      html
        .find("#complex-stuff")
        .html(
          `<p>*This is a complex enchantment with <b>${
            Object.entries(spell.system.components).length
          }</b> components. You will attempt to cast each one in sequence. If any step fails, the GM decides what ultimately happens.</p>`
        );
    }

    if (
      spell.system.keywords.includes("Static") ||
      actor.type == "Creature" ||
      prepared
    ) {
      html.find("#amplify-info").hide();
    ...
    if (
      ...
      (spell.type == "Enchantment" &&
        Object.keys(spell.system.materialCosts).length > 0)
    ) {
      ...
      const canUseAlchemistsPouch =
        hasAlchemistsPouch &&
        actor.system.coreFeatures.alchemistsPouch.configuration.maxRarity >=
          spell.system.rarity &&
        actor.hasResource("Alchemist's Pouch", 1);
      if (hasMaterials) {
        html
          .find("#materialStatus")
          .html(`<strong style="color: green">In Inventory</strong>`);
      } else if (hasAlchemistsPouch && canUseAlchemistsPouch) {
        html
          .find("#materialStatus")
          .html(`<strong>In Alchemist's Pouch</strong>`);
      } else {
        html
          .find("#materialStatus")
          .html(`<strong style="color: red">Missing Materials</strong>`);
      }
      html.find("#materialCosts").html(`
          <p>This spell requires materials. Choose where to source your materials for casting.</p>
            <select id="materialSource">
              ...
              ${
                hasAlchemistsPouch
                  ? canUseAlchemistsPouch
                    ? `<option value="1">Alchemist's Pouch</option>`
                    : `<option disabled>Alchemist's Pouch</option>`
                  : ""
              }
  ...
  static _getComplexPercentage(enchantment, actor, ampFactor) {
    ...
    for (const component of Object.values(enchantment.system.components)) {
      ...
      const passiveSpellSkill =
        ...
        (form == "genericCast"
          ? actor.system.casterLevel
          : actor.system.magic[form].mod);
      const stepProb = Math.min(
        1.0,
        0.55 + (passiveSpellSkill - difficulty) * 0.05
      );
      prob *= stepProb;
      console.log(
        `[DEBUG] component form=${form} level=${level} skill=${spellSkillLevel} diff=${difficulty} passive=${passiveSpellSkill} prob=${stepProb} cumulative=${prob}`
      );
    }
  ...
  static enchantItem(actor, enchantment, ampFactor, isPrepared, pool) {
    ...
    if (
      Object.keys(enchantment.system.validTargets).filter(
        (t) => enchantment.system.validTargets[t]
      ) == ["creature"]
    ) {
  ...
  static async useInspiration(actor) {
    if (!actor.system.inspiration) {
      ui.notifications.warn(
        `${actor.name} doesn't have any points of inspiration.`
      );
      return;
    }
    new Dialog({
      ...
      buttons: {
        luckyRoll: {
          ...
          callback: () => {
            ...
            actor.actionMessage(
              actor.img,
              `${NEWERA.images}/inspiration.png`,
              `<b>Lucky Roll.</b> You spend a point of Inspiration to gain a +7 Luck bonus to your next d20 check. If that roll results in a critical failure, it's treated as a normal failure instead.`
            );
          },
        },
        reroll: {
          ...
          callback: () => {
            ...
            actor.actionMessage(
              actor.img,
              `${NEWERA.images}/inspiration.png`,
              `<b>Reroll.</b> You spend a point of Inspiration to reroll any die roll you just made. You must take the second result, even if it's worse.`
            );
          },
        },
        extraFrame: {
          ...
          callback: () => {
            ...
            actor.actionMessage(
              actor.img,
              `${NEWERA.images}/inspiration.png`,
              `<b>Extra Frame.</b> You spend a point of Inspiration to gain one additional action or reaction frame in the current turn.`
            );
          },
        },
        cancel: {
          ...
          callback: (html) => {
            ...
            for (const actor of playerCharacters) {
              ...
              phones.forEach((i) => {
                ...
                count++;
                console.log(
                  `Added contact to ${i.name} in inventory of ${actor.name}`
                );
              });
            }
            ui.notifications.info(
              `${name} has been added to all player character phone contacts (${count} total devices.)`
            );
  ...
  static async advanceGameClock(time = {}) {
    ...
    ui.notifications.info(
      `The time is now ${now.year}-${now.month}-${now.day} ${now.hour}:${now.minute}.`
    );
  }
  ...
  static async randomizeWeather(weather) {
    const currentWeather = await game.settings.get(
      "newera-sol366",
    ...
    if (weather == 0) {
      ...
      await game.settings.set(
        "newera-sol366",
        "world.weather.conditions",
        conditionSequence[newIndex]
      );
    } else if (weather == 2) {
      //Random
      const newWeather =
        conditionSequence[Math.floor(Math.random() * conditionSequence.length)];
      await game.settings.set(
        "newera-sol366",
        "world.weather.conditions",
        newWeather
      ...
      new Dialog({
        ...
        render: (html) => {
          ...
          html.find("#quantity-up").click(() => {
            quantity = Math.min(
              allowAll ? item.system.quantity : item.system.quantity - 1,
              quantity + 1
            );
            html.find("#quantity").text(quantity);
          });
  ...
  static async reloadRangedWeapon(actor, item) {
    ...
    if (["SA", "FA"].includes(item.system.firingAction)) {
      if (item.system.ammo.loaded == item.system.ammo.clipSize) {
        ui.notifications.info(
          `Reloaded ${item.name} with ${loaded} rounds of ${ammo.name}.`
        );
      } else {
        ui.notifications.info(
          `Loaded ${loaded} rounds of ${ammo.name} into ${item.name}.`
        );
      }
      actor.actionMessage(
        actor.img,
